...
export async function GET() {
  ...
  const withSma: { symbol: string; close: number; pct: number; volume: number }[] = [];
  for (const candidate of candidates) {
    const candles = await fetchHistoricalCandles(candidate.key, token, HIST_LOOKBACK_DAYS);
    if (!candles || candles.length === 0) continue;
    const smaData = computeSma(candles, SMA_PERIOD, 5);
    if (!smaData) continue;
    if (smaData.latest <= smaData.sma * 1.5) continue;
    withSma.push({
