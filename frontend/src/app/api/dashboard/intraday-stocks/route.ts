...
export async function GET() {
  ...
  for (const candidate of candidates) {
    ...
    const closeSma10 = computeSma(candles, 10, 4);
    const closeSma20 = computeSma(candles, 20, 4);
    if (!volSma || !closeSma10 || !closeSma20) continue;
    if (volSma.latest <= volSma.sma * 2) continue;
    if (candidate.close <= closeSma10.sma || candidate.close <= closeSma20.sma) continue;

    const intraday = await fetchIntradayCandles(candidate.key, token);
    ...
    const prevLow = Number(prev[3] ?? 0);
    if (!latestClose || !prevLow) continue;
    if (latestClose <= prevLow) continue;
